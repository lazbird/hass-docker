...
from .const import (
    ...
    LECOO_RESPONSE_CODE,
    LECOO_RESPONSE_MSG,
    LECOO_RESPONSE_QR_CODE,
    LECOO_RESPONSE_RESULT,
    LECOO_RESPONSE_SUCCESS,
    LECOO_SCHEMA,
)


class LecooConfigFlow(ConfigFlow, domain=DOMAIN):
    """Lecoo aiot config flow."""
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            placeholders = {
                LECOO_RESPONSE_MSG: response.get(LECOO_RESPONSE_MSG, "Unknown error"),
                LECOO_RESPONSE_CODE: response.get(LECOO_RESPONSE_CODE, "0"),
            }
        else:
            user_input = {}
        ...
        if not ret:
            ...
            return self.async_show_form(
                ...
                description_placeholders={
                    LECOO_RESPONSE_MSG: info.get(LECOO_RESPONSE_MSG, "Unknown error"),
                    LECOO_RESPONSE_CODE: info.get(LECOO_RESPONSE_CODE, 0),
                },
            )
    ...
    async def async_step_reauth(
        ...
        """Handle initiation of re-authentication with Lecoo aiot."""
    ...
    async def async_step_reauth_user_code(
        ...
        """Handle re-authentication with a Lecoo aiot."""
        errors = {}
        placeholders = {}
    ...
    async def __async_get_qr_code(self, user_code: str) -> tuple[bool, dict[str, Any]]:
        ...
        if success := response.get(LECOO_RESPONSE_SUCCESS, False):
            self.__user_code = user_code
            self.__qr_code = response[LECOO_RESPONSE_RESULT][LECOO_RESPONSE_QR_CODE]
