"""Support for the Lecoo aiot lights."""
...
from .const import LECOO_DISCOVERY_NEW, DeviceCategory, DPCode, WorkMode
from .coordinator import LecooConfigEntry
from .entity import LecooEntity


@dataclass(frozen=True)
class LecooLightEntityDescription(LightEntityDescription):
    """Describe an Lecoo aiot light entity."""
...
LIGHTS: dict[DeviceCategory, tuple[LecooLightEntityDescription, ...]] = {
    DeviceCategory.BZYD: (
        LecooLightEntityDescription(
...
async def async_setup_entry(
    ...
    """Set up Lecoo aiot light dynamically through Lecoo aiot discovery."""
    manager = entry.runtime_data.manager

    @callback
    def async_discover_device(device_ids: list[str]):
        """Discover and add a discovered Lecoo aiot light."""
        entities: list[LecooLightEntity] = []
        for device_id in device_ids:
            device = manager.device_map[device_id]
            if descriptions := LIGHTS.get(device.category):
                entities.extend(
                    LecooLightEntity(device, manager, description, definition)
...
class LecooLightEntity(LecooEntity, LightEntity):
    """Lecoo aiot light device."""

    entity_description: LecooLightEntityDescription
    ...
    def __init__(
        ...
        description: LecooLightEntityDescription,
        definition: TuyaLightDefinition,
    ) -> None:
        """Init Lecoo aiotHaLight."""
