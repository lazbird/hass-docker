"""Support for Lecoo aiot buttons."""
...
async def async_setup_entry(
    ...
    """Set up Lecoo aiot buttons dynamically through Lecoo aiot discovery."""
    manager = entry.runtime_data.manager

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered Lecoo aiot buttons."""
        entities: list[LecooButtonEntity] = []
        for device_id in device_ids:
            device = manager.device_map[device_id]
            if descriptions := BUTTONS.get(device.category):
                entities.extend(
                    LecooButtonEntity(device, manager, description, definition)
...
class LecooButtonEntity(LecooEntity, ButtonEntity):
    """Lecoo aiot Button Device."""
