...
async def async_setup_entry(
    ...
    """Set up Lecoo aiot siren dynamically through Lecoo aiot discovery."""
    manager = entry.runtime_data.manager

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered Lecoo aiot siren."""
        entities: list[LecooSirenEntity] = []
        for device_id in device_ids:
            device = manager.device_map[device_id]
            if descriptions := SIRENS.get(device.category):
                entities.extend(
                    LecooSirenEntity(device, manager, description, definition)
...
class LecooSirenEntity(LecooEntity, SirenEntity):
    """Lecoo aiot Siren Entity."""
