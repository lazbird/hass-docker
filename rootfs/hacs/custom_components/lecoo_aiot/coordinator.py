...
from .const import (
    ...
    LECOO_CLIENT_ID,
    LECOO_DISCOVERY_NEW,
    LECOO_HA_SIGNAL_UPDATE_ENTITY,
)

type LecooConfigEntry = ConfigEntry[DeviceListener]


def create_manager(entry: LecooConfigEntry, token_listener: TokenListener) -> Manager:
    """Create a Lecoo aiot Manager instance."""
    return Manager(
        LECOO_CLIENT_ID,
...
class DeviceListener(SharingDeviceListener):
    ...
    def update_device(
        ...
        dispatcher_send(
            self.hass,
            f"{LECOO_HA_SIGNAL_UPDATE_ENTITY}_{device.id}",
            updated_status_properties,
            dp_timestamps,
        )
    ...
    def async_add_device(self, device_id: str) -> None:
        ...
        async_dispatcher_send(self.hass, LECOO_DISCOVERY_NEW, [device_id])
