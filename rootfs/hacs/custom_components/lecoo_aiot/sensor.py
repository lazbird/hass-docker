"""Support for Lecoo aiot sensors."""
...
class LecooSensorEntityDescription(SensorEntityDescription):
    """Describes Lecoo aiot sensor entity."""
...
BATTERY_SENSORS: tuple[LecooSensorEntityDescription, ...] = (
    LecooSensorEntityDescription(
...
SENSORS: dict[DeviceCategory, tuple[LecooSensorEntityDescription, ...]] = {
    DeviceCategory.AQCZ: (
        LecooSensorEntityDescription(
    ...
    DeviceCategory.DGNBJ: (
        ...
        LecooSensorEntityDescription(
            ...
            # pH is unitless; treat any Lecoo aiot-reported "pH"/"ph" unit as unitless
            state_class=SensorStateClass.MEASUREMENT,
        ),
        LecooSensorEntityDescription(
...
async def async_setup_entry(
    ...
    """Set up Lecoo aiot sensor dynamically through Lecoo aiot discovery."""
    manager = entry.runtime_data.manager

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered Lecoo aiot sensor."""
        entities: list[LecooSensorEntity] = []
        for device_id in device_ids:
            device = manager.device_map[device_id]
            if descriptions := SENSORS.get(device.category):
                entities.extend(
                    LecooSensorEntity(device, manager, description, definition)
...
class LecooSensorEntity(LecooEntity, SensorEntity):
    """Lecoo aiot Sensor Entity."""

    entity_description: LecooSensorEntityDescription

    def __init__(
        self,
        device: CustomerDevice,
        device_manager: Manager,
        description: LecooSensorEntityDescription,
        definition: TuyaSensorDefinition,
    ) -> None:
        """Init Lecoo aiot sensor."""
