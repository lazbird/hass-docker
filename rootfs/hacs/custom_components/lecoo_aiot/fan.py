...
_LECOO_TO_HA_DIRECTION_MAPPINGS = {
    TuyaFanDirection.FORWARD: DIRECTION_FORWARD,
    TuyaFanDirection.REVERSE: DIRECTION_REVERSE,
}
_HA_TO_LECOO_DIRECTION_MAPPINGS = {
    v: k for k, v in _LECOO_TO_HA_DIRECTION_MAPPINGS.items()
}
...
async def async_setup_entry(
    ...
    """Set up Lecoo aiot fan dynamically through Lecoo aiot discovery."""
    manager = entry.runtime_data.manager

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered Lecoo aiot fan."""
        entities: list[LecooFanEntity] = []
        for device_id in device_ids:
            device = manager.device_map[device_id]
            if (description := FANS.get(device.category)) and (
                definition := get_default_definition(device)
            ):
                entities.append(LecooFanEntity(device, manager, description, definition))
...
class LecooFanEntity(LecooEntity, FanEntity):
    """Lecoo aiot Fan Device."""
    ...
    async def async_set_direction(self, direction: str) -> None:
        ...
        if lecoo_value := _HA_TO_LECOO_DIRECTION_MAPPINGS.get(direction):
            await self._async_send_wrapper_updates(self._direction_wrapper, lecoo_value)
    ...
    def current_direction(self) -> str | None:
        ...
        lecoo_value = self._read_wrapper(self._direction_wrapper)
        return _LECOO_TO_HA_DIRECTION_MAPPINGS.get(lecoo_value) if lecoo_value else None
