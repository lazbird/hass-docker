"""Services for Lecoo aiot integration."""
...
def _get_lecoo_device(
    hass: HomeAssistant, device_id: str
) -> tuple[CustomerDevice, Manager]:
    """Get a Lecoo aiot device and manager from a Home Assistant device registry ID."""
    ...
    # Find the Lecoo aiot device ID from identifiers
    lecoo_device_id = None
    for identifier_domain, identifier_value in device_entry.identifiers:
        if identifier_domain == DOMAIN:
            lecoo_device_id = identifier_value
            break

    if lecoo_device_id is None:
        raise ServiceValidationError(
            translation_domain=DOMAIN,
            translation_key="device_not_lecoo_device",
            translation_placeholders={
                "device_id": device_id,
            },
        )

    # Find the device in Lecoo aiot config entry
    for entry in hass.config_entries.async_loaded_entries(DOMAIN):
        manager = entry.runtime_data.manager
        if lecoo_device_id in manager.device_map:
            return manager.device_map[lecoo_device_id], manager
...
async def async_get_feeder_meal_plan(
    ...
    device, _ = _get_lecoo_device(call.hass, call.data[ATTR_DEVICE_ID])
...
async def async_set_feeder_meal_plan(call: ServiceCall) -> None:
    ...
    device, manager = _get_lecoo_device(call.hass, call.data[ATTR_DEVICE_ID])
