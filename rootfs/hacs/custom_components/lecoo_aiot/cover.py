...
class LecooCoverEntityDescription(CoverEntityDescription):
    """Describe a Lecoo aiot cover entity."""
...
COVERS: dict[DeviceCategory, tuple[LecooCoverEntityDescription, ...]] = {
    DeviceCategory.CKMKZQ: (
        LecooCoverEntityDescription(
            key=DPCode.SWITCH_1,
...
async def async_setup_entry(
    ...
    """Set up Lecoo aiot cover dynamically through Lecoo aiot discovery."""
    manager = entry.runtime_data.manager

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered Lecoo aiot cover."""
        entities: list[LecooCoverEntity] = []
        for device_id in device_ids:
            device = manager.device_map[device_id]
            if descriptions := COVERS.get(device.category):
                entities.extend(
                    LecooCoverEntity(device, manager, description, definition)
...
class LecooCoverEntity(LecooEntity, CoverEntity):
    """Lecoo aiot Cover Device."""

    entity_description: LecooCoverEntityDescription

    def __init__(
        self,
        device: CustomerDevice,
        device_manager: Manager,
        description: LecooCoverEntityDescription,
        definition: TuyaCoverDefinition,
    ) -> None:
        """Init Lecoo aiot Cover."""
