"""Support for Lecoo aiot Smart devices."""
...
from .coordinator import DeviceListener, TokenListener, LecooConfigEntry, create_manager
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    """Set up the Lecoo aiot Services."""
    await async_setup_services(hass)

    return True


async def async_setup_entry(hass: HomeAssistant, entry: LecooConfigEntry) -> bool:
    ...
    # Get all devices from Lecoo aiot
    try:
...
async def cleanup_device_registry(
    hass: HomeAssistant, device_manager: Manager, entry: LecooConfigEntry
) -> None:
...
async def async_unload_entry(hass: HomeAssistant, entry: LecooConfigEntry) -> bool:
    """Unloading the Lecoo aiot platforms."""
...
async def async_remove_entry(hass: HomeAssistant, entry: LecooConfigEntry) -> None:
    """Remove a config entry.

    This will revoke the credentials from Lecoo aiot.
    """
    manager = Manager(
        LECOO_CLIENT_ID,
