"""Lecoo aiot Home Assistant Base Device Model."""
...
from .const import DOMAIN, LOGGER, LECOO_HA_SIGNAL_UPDATE_ENTITY


class LecooEntity(Entity):
    """Lecoo aiot base device."""
    ...
    def __init__(
        ...
        # LecooEntity initialize mq can subscribe
        device.set_up = True
        self.device = device
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self.async_on_remove(
            async_dispatcher_connect(
                self.hass,
                f"{LECOO_HA_SIGNAL_UPDATE_ENTITY}_{self.device.id}",
    ...
    async def _handle_state_update(
        ...
        """Called when Lecoo aiot device sends an update."""
        if (
