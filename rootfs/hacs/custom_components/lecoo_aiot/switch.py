"""Support for Lecoo aiot switches."""
...
SWITCHES: dict[DeviceCategory, tuple[SwitchEntityDescription, ...]] = {
    ...
    DeviceCategory.MAL: (
        SwitchEntityDescription(
            ...
            # This switch is called "Arm Beep" in the official Lecoo aiot app
            translation_key="arm_beep",
            entity_category=EntityCategory.CONFIG,
        ),
        SwitchEntityDescription(
            key=DPCode.SWITCH_ALARM_LIGHT,
            # This switch is called "Siren" in the official Lecoo aiot app
...
async def async_setup_entry(
    ...
    """Set up Lecoo aiot sensors dynamically through Lecoo aiot discovery."""
    manager = entry.runtime_data.manager

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered Lecoo aiot sensor."""
        entities: list[LecooSwitchEntity] = []
        for device_id in device_ids:
            device = manager.device_map[device_id]
            if descriptions := SWITCHES.get(device.category):
                entities.extend(
                    LecooSwitchEntity(device, manager, description, definition)
...
class LecooSwitchEntity(LecooEntity, SwitchEntity):
    """Lecoo aiot Switch Device."""
