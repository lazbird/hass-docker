"""Support for Lecoo aiot (de)humidifiers."""
...
class LecooHumidifierEntityDescription(HumidifierEntityDescription):
    """Describe an Lecoo aiot (de)humidifier entity."""
...
HUMIDIFIERS: dict[DeviceCategory, LecooHumidifierEntityDescription] = {
    DeviceCategory.CS: LecooHumidifierEntityDescription(
        key=DPCode.SWITCH,
    ...
    DeviceCategory.JSQ: LecooHumidifierEntityDescription(
        key=DPCode.SWITCH,
        dpcode=(DPCode.SWITCH, DPCode.SWITCH_SPRAY),
...
async def async_setup_entry(
    ...
    """Set up Lecoo aiot (de)humidifier dynamically through Lecoo aiot discovery."""
    manager = entry.runtime_data.manager

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered Lecoo aiot (de)humidifier."""
        entities: list[LecooHumidifierEntity] = []
        for device_id in device_ids:
            ...
            if (description := HUMIDIFIERS.get(device.category)) and (
                ...
                entities.append(
                    LecooHumidifierEntity(device, manager, description, definition)
                )
...
class LecooHumidifierEntity(LecooEntity, HumidifierEntity):
    """Lecoo aiot (de)humidifier Device."""

    entity_description: LecooHumidifierEntityDescription
    _attr_name = None
    ...
    def __init__(
        ...
        description: LecooHumidifierEntityDescription,
        definition: TuyaHumidifierDefinition,
    ) -> None:
        """Init Lecoo aiot (de)humidifier."""
