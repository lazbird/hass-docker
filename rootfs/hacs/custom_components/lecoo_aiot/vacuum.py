"""Support for Lecoo aiot Vacuums."""
...
_LECOO_TO_HA_ACTIVITY_MAPPINGS = {
...
async def async_setup_entry(
    ...
    """Set up Lecoo aiot vacuum dynamically through Lecoo aiot discovery."""
    manager = entry.runtime_data.manager

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered Lecoo aiot vacuum."""
        entities: list[LecooVacuumEntity] = []
...
class LecooVacuumEntity(LecooEntity, StateVacuumEntity):
    """Lecoo aiot Vacuum Device."""
    ...
    def activity(self) -> VacuumActivity | None:
        """Return Lecoo aiot vacuum device state."""
        lecoo_value = self._read_wrapper(self._activity_wrapper)
        return _LECOO_TO_HA_ACTIVITY_MAPPINGS.get(lecoo_value) if lecoo_value else None
