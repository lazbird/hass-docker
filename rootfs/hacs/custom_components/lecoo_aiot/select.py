...
async def async_setup_entry(
    ...
    """Set up Lecoo aiot select dynamically through Lecoo aiot discovery."""
    manager = entry.runtime_data.manager

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered Lecoo aiot select."""
        entities: list[LecooSelectEntity] = []
        for device_id in device_ids:
            device = manager.device_map[device_id]
            if descriptions := SELECTS.get(device.category):
                entities.extend(
                    LecooSelectEntity(device, manager, description, definition)
...
class LecooSelectEntity(LecooEntity, SelectEntity):
    """Lecoo aiot Select Entity."""
    ...
    def __init__(
        ...
        """Initialize a Lecoo aiot select entity."""
