...
async def async_setup_entry(
    ...
    """Set up Lecoo aiot valves dynamically through Lecoo aiot discovery."""
    manager = entry.runtime_data.manager

    @callback
    def async_discover_device(device_ids: list[str]) -> None:
        """Discover and add a discovered Lecoo aiot valve."""
        entities: list[LecooValveEntity] = []
        for device_id in device_ids:
            device = manager.device_map[device_id]
            if descriptions := VALVES.get(device.category):
                entities.extend(
                    LecooValveEntity(device, manager, description, definition)
...
class LecooValveEntity(LecooEntity, ValveEntity):
    """Lecoo aiot Valve Device."""
