"""Diagnostics support for Lecoo aiot."""
...
async def async_get_config_entry_diagnostics(
    hass: HomeAssistant, entry: LecooConfigEntry
) -> dict[str, Any]:
...
async def async_get_device_diagnostics(
    hass: HomeAssistant, entry: LecooConfigEntry, device: DeviceEntry
) -> dict[str, Any]:
...
def _async_get_diagnostics(
    ...
    if device:
        lecoo_device_id = next(iter(device.identifiers))[1]
        data |= _async_device_as_dict(hass, manager.device_map[lecoo_device_id])
    else:
        data.update(
            devices=[
...
def _async_device_as_dict(
    ...
    """Represent a Lecoo aiot device as a dictionary."""
    ...
    # Gather information how this Lecoo aiot device is represented in Home Assistant
